...
function ammunition(estilo1, estilo2) {
    ...
    var frontPage = (document.getElementsByTagName("body")[0].className =
        estail);
    console.log(estail);
}
...
function hasAccess(detail, className) {
    if (detail.classList) return detail.classList.contains(className);
    return !!detail.className.match(
        new RegExp("(\\s|^)" + className + "(\\s|$)")
    );
}

function openAccess(detail, className) {
    if (detail.classList) detail.classList.add(className);
    else if (!hasClass(detail, className)) detail.className += " " + className;
}

function closeAccess(detail, className) {
    if (detail.classList) detail.classList.remove(className);
...
/*function accesstab() {
    ...
    if (
        document.getElementById("panel-accesibilidad").style.display === "none"
    ) {
        document
            .getElementById("panel-accesibilidad")
            .setAttribute("style", "display:block");
        openAccess(mainwin, "accesibility-on");
    } else if (
        document.getElementById("panel-accesibilidad").style.display === "block"
    ) {
        document
            .getElementById("panel-accesibilidad")
...
function accesstab() {
    var mainwin = document.getElementsByClassName("detalle")[0];
    var panelAccesibilidad = document.getElementById("panel-accesibilidad");
    var panelAccesibilidadStyle = getComputedStyle(panelAccesibilidad);

    if (panelAccesibilidadStyle.display === "none") {
        panelAccesibilidad.style.display = "block";
        openAccess(mainwin, "accesibility-on");
    } else if (panelAccesibilidadStyle.display === "block") {
...
function defaultConfig() {
    ...
    style.type = "text/css";
    if (style.styleSheet) {
        style.styleSheet.cssText = css;zz
    } else {
        style.appendChild(document.createTextNode(css));
    }
    head.appendChild(style);
...
window.onload = function () {
    document
        .getElementById("pestania-accesibilidad")
        .addEventListener("click", accesstab);
};
